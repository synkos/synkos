      ...
      <Transition name="signout-flip" mode="out-in">
        ...
        <div v-if="signOutState === 'confirm'" key="confirm" class="signout-confirm">
          <div class="signout-icon-wrap">
            <AppIcon name="logout" size="26px" style="color: #ff453a" />
          </div>
          ...
          <button
            ...
            @click="$emit('confirm')"
          >
            <AppSpinner v-if="isProcessing" size="18px" color="white" />
            <span v-else>{{
...
import { AppBottomSheet, AppIcon, AppSpinner, useSheetDrag } from '@synkos/ui';
