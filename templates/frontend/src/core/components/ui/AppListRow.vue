<template>
  <component
    :is="value ? 'div' : 'button'"
    class="app-list-row"
    :disabled="!value ? disabled : undefined"
    @click="!value && !disabled ? $emit('click') : undefined"
  >
    <!-- Icon -->
    <div v-if="icon" class="row-icon-wrap" :style="{ background: iconBg }">
      <q-icon :name="icon" size="18px" :style="{ color: iconColor }" />
    </div>

    <!-- Label + hint -->
    <div class="row-body">
      <span
        class="row-label"
        :class="{ 'row-label--muted': disabled && !value, 'row-label--danger': danger }"
        >{{ label }}</span
      >
      <span v-if="hint" class="row-hint">{{ hint }}</span>
    </div>

    <!-- Right: slot > value > badge+chevron > chevron -->
    <slot name="right">
      <template v-if="value">
        <span class="row-value">{{ value }}</span>
      </template>
      <template v-else>
        <span v-if="comingSoon" class="badge-soon">{{ t('common.comingSoon') }}</span>
        <q-icon name="chevron_right" class="row-chevron" />
      </template>
    </slot>
  </component>
</template>

<script setup lang="ts">
import { useI18n } from 'vue-i18n';

interface Props {
  // Icon (optional)
  icon?: string;
  iconColor?: string;
  iconBg?: string;
  // Content
  label: string;
  hint?: string;
  // State
  disabled?: boolean;
  danger?: boolean;
  // Right side
  comingSoon?: boolean;
  value?: string; // static info value — renders as div, no chevron
}

defineProps<Props>();
defineEmits<{ click: [] }>();

const { t } = useI18n();
</script>

<style lang="scss" scoped>
.app-list-row {
  display: flex;
  align-items: center;
  gap: $space-6;
  padding: $space-6 $space-8;
  min-height: 52px;
  background: transparent;
  border: none;
  width: 100%;
  text-align: left;
  -webkit-tap-highlight-color: transparent;
  cursor: default;
}

button.app-list-row {
  cursor: pointer;

  &:active:not(:disabled) {
    background: $surface-press;
  }
  &:disabled {
    opacity: 0.45;
    cursor: default;
  }
}

// ── Icon ──────────────────────────────────────────────────────────
.row-icon-wrap {
  width: 32px;
  height: 32px;
  border-radius: $radius-sm;
  display: flex;
  align-items: center;
  justify-content: center;
  flex-shrink: 0;
}

// ── Body ──────────────────────────────────────────────────────────
.row-body {
  flex: 1;
  display: flex;
  flex-direction: column;
  gap: $space-1;
  min-width: 0;
}

.row-label {
  font-size: $font-body;
  font-weight: 400;
  color: $text-secondary;
  letter-spacing: $ls-normal;

  &--muted {
    color: $text-disabled;
  }
  &--danger {
    color: $negative;
  }
}

.row-hint {
  font-size: $font-caption;
  color: $text-tertiary;
  letter-spacing: $ls-normal;
  line-height: 1.3;
}

// ── Right ─────────────────────────────────────────────────────────
.row-value {
  font-size: $font-body;
  color: rgba(235, 235, 245, 0.42);
  letter-spacing: $ls-normal;
  flex-shrink: 0;
}

.row-chevron {
  color: rgba(255, 255, 255, 0.22) !important;
  font-size: 20px !important;
  flex-shrink: 0;
}

.badge-soon {
  font-size: $font-xs;
  font-weight: 600;
  color: $text-label;
  background: $separator;
  border: 0.5px solid $border-medium;
  border-radius: $radius-full;
  padding: $space-1 $space-4;
  letter-spacing: $ls-caps;
  text-transform: uppercase;
  white-space: nowrap;
}
</style>
