...
export const UsernameController = {
  ...
  checkAvailability: async (req: Request, res: Response) => {
    ...
    const suggestions = !result.available && !result.error ? await generateSuggestions(raw) : [];
  ...
  setUsername: async (req: Request, res: Response) => {
    ...
    const normalized = await setUsername(req.user._id.toString(), parsed.data.username);
  ...
  changeUsername: async (req: Request, res: Response) => {
    ...
    const normalized = await changeUsername(req.user._id.toString(), parsed.data.username);
