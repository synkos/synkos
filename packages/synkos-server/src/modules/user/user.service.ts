...
function validateAvatarFile(file: Express.Multer.File): void {
  if (!ALLOWED_MIME_TYPES.has(file.mimetype)) {
    throw Object.assign(new Error('Only JPEG, PNG, WebP, or GIF images are accepted.'), {
      status: 422,
      code: 'INVALID_FILE_TYPE',
    });
  }
  if (file.size > MAX_AVATAR_BYTES) {
    throw Object.assign(new Error('Image must be smaller than 5 MB.'), {
      status: 422,
      code: 'FILE_TOO_LARGE',
    });
  }
...
export async function updateUsername(userId: string, rawUsername: string): Promise<PublicUser> {
  ...
  try {
    ...
    if (e.status === 429) {
      throw makeError(
        e.message ?? 'Username change not allowed yet.',
        429,
 ...
 */
export async function updatePhoto(userId: string, file?: Express.Multer.File): Promise<PublicUser> {
  const user = await User.findById(userId);
  if (!user) throw makeError('User not found.', 404, 'USER_NOT_FOUND');
  ...
  logAuditEvent({
    userId,
    type: 'photo_updated',
    oldValue: previousKey ?? undefined,
    newValue: url,
  });
